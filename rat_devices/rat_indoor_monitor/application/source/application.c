...
//     - Add Temp & Hum 15 Click (MIKROE-4496) to mikroBUS 1.
...
static rat_trh_measurement_t measurement;
...
void application_setup (void)
{
    ...
    if (!rat_trh_add(rat_trh_base_address(), &app_sensor_identifier)) {
        application_error();
    }
...
void application_measurement (void)
{
    ...
    if (!rat_trh_measure(app_sensor_identifier, &measurement)) {
        application_error();
    }
}
...
void application_transmission (void)
{
    ...
    tx_payload[0] = (temperature >> 16) & 0xFF;
    tx_payload[1] = (temperature >>  8) & 0xFF;
    tx_payload[2] = (temperature >>  0) & 0xFF;

    tx_payload[3] = (humidity    >>  8) & 0xFF;
    tx_payload[4] = (humidity    >>  0) & 0xFF;
